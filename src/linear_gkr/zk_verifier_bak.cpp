...
void zk_verifier::broadcast(int r){
	...
	//output layer
	{
		prime_field::field_element random_r=prime_field::field_element(1);
		std::vector<prime_field::field_element> real_output_layer;
		std::vector<prime_field::field_element> beta_multiplier;
		beta_multiplier.resize(r);
		real_output_layer.resize(1 << C.circuit[C.total_depth - 2].bit_length);
		alpha_beta_sum_output_layer.resize(1 << C.circuit[C.total_depth - 2].bit_length);
		for(int i = 0; i < (1 << C.circuit[C.total_depth - 2].bit_length); ++i)
		{
		...
		for(int i = 0; i < C.circuit[C.total_depth - 1].bit_length; ++i)
		{
			...
			for(int j1 = 0; j1 < r; ++j1)
			{
				...
				int a=index_array[j1]&(1 << (i));
				a=a>>i;
				if(a==0){
				//this bit is zero
				    beta_part_one = 0;
				    beta_part_zero = beta_multiplier[r];
				}
				else{
				//this bit is one
				    beta_part_zero = 0;
				    beta_part_one = beta_multiplier[r];
				}
				beta_part.b = beta_part_zero, beta_part.a = beta_part_one - beta_part_zero;
				response[r] = response[r] + v_part * beta_part;
				quadratic_poly response_tmp(0,0,0);
				response_tmp.a=response[r].a*random_tmp;
				response_tmp.b=response[r].b*random_tmp;
				response_tmp.c=response[r].c*random_tmp;
				random_tmp=random_tmp*random_r;
				response_sum=response_sum+response_tmp;
			}
			for(int j = 0; j < r; ++j)
			{
				assert(response[j].eval((index_array[j] >> i) & 1) == alpha_beta_sum_output_layer[j]);
				verifiers_fs[j].update((const char*)&response[j], sizeof(quadratic_poly));
			}
			...
			for(int j1 = 0; j1 < r; ++j1)
			{
				int j=index_array[i]>>(i+1);
				v_part_one = real_output_layer[j << 1 | 1];
				v_part_zero = real_output_layer[j << 1];
				v_part.b = v_part_zero;
				v_part.a = v_part_one - v_part_zero;
				real_output_layer[j1] = v_part.eval(new_r);
			}
		}
	}
