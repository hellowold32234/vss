...
#include <stdio.h>
using namespace std;
void sm3_hash( char *message, size_t message_len, unsigned char *digest) {
...
void zk_verifier::broadcast(int r){
       auto output_t0 = std::chrono::high_resolution_clock::now();
	std::vector<prime_field::field_element> alpha_beta_sum_output_layer;
	prime_field::field_element alpha_beta_sum_output_layer_sum;
	//std::vector<prime_field::field_element> r_0, one_minus_r_0, r_1, one_minus_r_1;
	//std::vector<prime_field::field_element> r_u, one_minus_r_u, r_v, one_minus_r_v;
	//initial random value
	prime_field::field_element alpha_beta_sum;
	if(r<=0){
			printf("The number of people accused of less than 1 does not implement that part of the agreement.");
			return ;
		}
	else if(r>(1<<p->log_num_degree)){
	printf("The number of people accused of more than t does not implement that part of the agreement.");
			return ;
	}
	int *index_array=new int [r];
	for(int i=0;i<r;i++){
        index_array[i]=i;
	}
	int size1 = 0;
        ...
        for (int i=0;i<r;i++) {
            size1 += std::to_string(index_array[i]).size();
        }
        for (int i=0;i<r;i++) {
            size1 += std::to_string(p -> circuit_value[C.total_depth - 2][index_array[i]].real).size();
        }

    // 创建字符数组并填充元素
    char* charArray = new char[size1 + 1];  // 额外留出一个位置存放 '\0'
    int index = 0;
    for (int i=0;i<r;i++) {
        std::string str = std::to_string(index_array[i]);
        for (char c : str) {
            charArray[index++] = c;
        }
    }
    for (int i=0;i<r;i++) {
        std::string str = std::to_string(p -> circuit_value[C.total_depth - 2][index_array[i]].real);
        for (char c : str) {
            charArray[index++] = c;
        }
    }
    charArray[size1] = '\0';  // 在末尾添加 '\0' 结束符
    unsigned char digest[EVP_MAX_MD_SIZE];

    // 调用封装的函数计算哈希值
    sm3_hash(charArray, strlen((char *)charArray), digest);
     /*std::cout << "字符数组: " << charArray << std::endl;
   for(int i = 0; i < (int)EVP_MD_size(EVP_sm3()); i++){
        printf("%02x", digest[i]);
    }
    printf("\n");*/
     char* endPtr;
    long long result = std::strtol(charArray, &endPtr, 16);
    int a= 0;
    std::vector<quadratic_poly>  polyproof;
    polyproof.resize(C.circuit[C.total_depth - 2].bit_length);
    std::vector<prime_field::field_element>  valueproof;
     valueproof.resize(C.circuit[C.total_depth - 2].bit_length);
	//output layer
	{       std::chrono::duration<double> total_time_span0(0);
	        std::chrono::duration<double> total_time_span1(0);
	        std::chrono::duration<double> total_time_span21(0);
	        std::chrono::duration<double> total_time_span22(0);
		prime_field::field_element random_r=prime_field::field_element(result);
		std::vector<prime_field::field_element> real_output_layer;
		std::vector<prime_field::field_element> beta_multiplier;
               beta_multiplier.resize(1 << C.circuit[C.total_depth - 2].bit_length);
		real_output_layer.resize(1 << C.circuit[C.total_depth - 2].bit_length);
		//
		("C.circuit[C.total_depth - 1].bit_length:%d\tC.circuit[C.total_depth - 2].bit_length:%d\n",C.circuit[C.total_depth - 1].bit_length,C.circuit[C.total_depth - 2].bit_length);
		alpha_beta_sum_output_layer.resize(r);
		for(int i = 0; i < (1 << C.circuit[C.total_depth - 2].bit_length); ++i)
		{
			real_output_layer[i] = p -> circuit_value[C.total_depth - 2][i];
		}
		
		prime_field::field_element tmp=random_r;
		for(int i=0;i<r;i++){
            beta_multiplier[i] = 1;
			alpha_beta_sum_output_layer[i] = p -> circuit_value[C.total_depth - 2][index_array[i]];
			alpha_beta_sum_output_layer_sum=alpha_beta_sum_output_layer_sum+tmp*alpha_beta_sum_output_layer[i];
			tmp=tmp*random_r;
		}
		//assert(C.circuit[C.total_depth - 1].bit_length == C.circuit[C.total_depth - 2].bit_length);
		int hello=C.circuit[C.total_depth - 1].bit_length;
		for(int i = 0; i < hello; ++i)
		{
			prime_field::field_element random_tmp=random_r;
			linear_poly beta_part, v_part;
			std::vector<quadratic_poly> response;
			quadratic_poly response_sum(0,0,0);
			a=sizeof(response_sum);
			prime_field::field_element v_part_one = 0, v_part_zero = 0;
			prime_field::field_element beta_part_one = 0, beta_part_zero = 0;
			v_part = linear_poly(0, 0);
			response.resize(r);
			int v_part_total_length = 1 << (hello - i);
			for(int j = 0; j < r; ++j)
			{
				response[j] = quadratic_poly(0, 0, 0);
			}
			...
			{
				int j=index_array[j1]>>(i+1);
				v_part_one = real_output_layer[j << 1 | 1];
				v_part_zero = real_output_layer[j << 1];
				v_part.b = v_part_zero;
				v_part.a = v_part_one - v_part_zero;
				int k=index_array[j1]&((1 << (i))-1);
				int a=(index_array[j1] >> i) & 1;
				//printf("a:%x\tk:%x\tj1:%d\n",a,k,j1);
 				if(a==0){
				//this bit is zero
				    beta_part_one = 0;
				    beta_part_zero = beta_multiplier[j1];
				}
				else{
				//this bit is one
				    beta_part_zero = 0;
				    beta_part_one = beta_multiplier[j1];
				}
				beta_part.b = beta_part_zero, beta_part.a = beta_part_one - beta_part_zero;
				response[j1] = response[j1] + v_part * beta_part;
				quadratic_poly response_tmp(0,0,0);
				response_tmp.a=response[j1].a*random_tmp;
				response_tmp.b=response[j1].b*random_tmp;
				response_tmp.c=response[j1].c*random_tmp;
				random_tmp=random_tmp*random_r;
				response_sum=response_sum+response_tmp;
			}
			auto output_t00 = std::chrono::high_resolution_clock::now();
			        assert((response_sum.eval(prime_field::field_element(0))+ response_sum.eval(prime_field::field_element(1)))== alpha_beta_sum_output_layer_sum);
			auto output_t11 = std::chrono::high_resolution_clock::now();
			auto output_t0 = std::chrono::high_resolution_clock::now();
			for(int j = 0; j < r; ++j)
			{
				assert((response[j].eval(0)+response[j].eval(1))== alpha_beta_sum_output_layer[j]);
				//verifiers_fs[j].update((const char*)&response[j], sizeof(quadratic_poly));
			}
			auto output_t1 = std::chrono::high_resolution_clock::now();
			auto output_t3 = std::chrono::high_resolution_clock::now();
			assert((response_sum.eval(prime_field::field_element(0))+ response_sum.eval(prime_field::field_element(1)))== alpha_beta_sum_output_layer_sum);
			auto output_t2 = std::chrono::high_resolution_clock::now();
			polyproof[i]=response_sum;
			valueproof[i]=alpha_beta_sum_output_layer_sum;
			total_time_span0+=output_t1-output_t0;
			total_time_span1+=output_t2-output_t3;
			auto output_t4 = std::chrono::high_resolution_clock::now();
			for(int j = 0; j < r; ++j)
			{
				verifiers_fs[j].update((const char*)&response[j], sizeof(quadratic_poly));
			}
			prime_field::field_element new_r;
			new_r = get_unified_hash(verifiers_fs.data(), 1 << log_num_verifier);
			auto output_t5 = std::chrono::high_resolution_clock::now();
			//total_time_span21+=output_t5-output_t4;
			//r_u.push_back(new_r);
			//one_minus_r_u.push_back(prime_field::field_element(1) - new_r);
			for(int k = 0; k < r; ++k)
			{
				if(((index_array[k]>>i)&1)==1){
				beta_multiplier[k] = beta_multiplier[k] * new_r;
				}
				else {
				beta_multiplier[k] = beta_multiplier[k] * (prime_field::field_element(1) - new_r);
				}
			}
			alpha_beta_sum_output_layer_sum=prime_field::field_element(0);
			//random_tmp=random_r;
			for(int j =0 ;j <r; ++j)
			{
				alpha_beta_sum_output_layer[j] = response[j].eval(new_r);
				//alpha_beta_sum_output_layer_sum=alpha_beta_sum_output_layer_sum+random_tmp*alpha_beta_sum_output_layer[j];
				//random_tmp=random_tmp*random_r;
			}
			alpha_beta_sum_output_layer_sum=response_sum.eval(new_r);
			if(v_part_total_length / 2 == 1)
			{
				alpha_beta_sum = v_part.eval(new_r);
			}
			...
			//printf("C.circuit[C.total_depth - 1].bit_length:%d\ta:%d\n",C.circuit[C.total_depth - 1].bit_length,i);
		}

		 std::cout <<r<<"\t"<<total_time_span0.count()<<"\t";//<< "Total s verifers time: " << total_time_span0.count() << " seconds" << std::endl;
		 std::cout << total_time_span1.count()<<"\t";//<< "Total one verifier time: " << total_time_span1.count() << " seconds" << std::endl;
		 std::cout << hello*r*(sizeof(polyproof[0])+sizeof(valueproof[0]))<<"\t";//"Size of s verifier: " << r*a << " bytes" << std::endl;
		 std::cout <<  hello*(sizeof(polyproof[0])+sizeof(valueproof[0]))<<"\n";//"Size of one verifie: " << a << " bytes" << std::endl;
		 /*for(int i=0;i<hello;i++){
		 printf("Sum-check Rounde%d\tpoly: %lld*x^2+%lld*x+%lld\tvalue:%lld\n",i,polyproof[i].a.real,polyproof[i].b.real,polyproof[i].c.real,valueproof[i].real);
		 }*/
